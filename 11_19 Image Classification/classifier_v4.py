...
def train_classifier():
    ...
    hog_features_train = [1000*extract_hog_features(image).reshape(-1) for image in data_train]
    hog_features_test = [1000*extract_hog_features(image).reshape(-1) for image in data_test]
    #print(len(hog_features_train[0]))

    # Concatenate features
    #features_train = color_hist_features_train
    #features_test = color_hist_features_test
    features_train = np.hstack((color_hist_features_train, hog_features_train))
    print('past train')
    features_test = np.hstack((color_hist_features_test, hog_features_test))
...
def extract_color_histogram(image):    
    ...
    avg_h = np.mean(image[:,:,0])
    avg_s = np.mean(image[:,:,1])
    avg_v = np.mean(image[:,:,2])
